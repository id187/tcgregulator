...
/** Gross revenue generated by paid events outside the regular release curve. */
function businessEventRevenueBonus(state: GameState): number {
  const revenueWon = activeBusinessRecords(state).reduce((bonus, record) => {
    switch (record.type) {
      case "beginner-camp":
        return bonus + totalUsers(state) * 250 + state.users.casual * 800;
      case "local-league":
        return bonus + totalUsers(state) * 200 + state.users.tier * 1_400;
      case "reprint-campaign":
        return bonus + totalUsers(state) * 650;
      case "collector-fair":
        return bonus + totalUsers(state) * 200 + state.users.collector * 4_500;
      case "tv-cm":
      case "animation-promotion":
      case "championship":
      case "store-tour":
  ...
  return revenueWon / 100_000_000 + resolvedEventBonus;
}
